...
const QuestionScreen: FC<Props> = () => {
  ...
  const isLastQuestion =
    parseInt(testState.currentQuestion.id) === questions?.length;
  ...
  const handleConfirmAnswer = () => {
    ...
    if (isLastQuestion) {
      ...
      const nextQuestion = questions?.find(
        (question: Question) =>
          question.id ===
          (parseInt(testState.currentQuestion.id) + 1).toString()
      ) as Question;
  ...
  const handlePrevAction = () => {
    const nextQuestion = questions?.find(
      (question: Question) =>
        question.id === (parseInt(testState.currentQuestion.id) - 1).toString()
    ) as Question;
    changeQuestion(nextQuestion);
  };

  useEffect(() => {
    const selectedUserAnswer = state.answers.find(
      (userAnswer: any) =>
        userAnswer.questionId === testState.currentQuestion.id
    ) as UserAnswer;
    if (selectedUserAnswer) {
      const selectedAnswer = testState.currentQuestion.answers.find(
        (answer: Answer) => answer.id === selectedUserAnswer.answerId
      ) as Answer;
  ...
  const renderNextButton = () => {
    return (
      <Button
        ...
        className="mt-5 md:whitespace-nowrap"
        variant="secondary"
      >
        {isLastQuestion
          ? translate("submit_btn_label")
          : translate("next_question_btn_label")}
      </Button>
    );
  };
  ...
  return (
    <TestBodyLayout title={testState.currentQuestion?.title}>
      <div className="flex flex-col px-5 mt-4">
        {testState.currentQuestion?.answers &&
          testState.currentQuestion?.answers.map((answer: Answer) => (
            <AnswerContainer
              key={answer?.id}
              answer={answer}
        ...
        <div
          className={`flex flex-row w-full ${
            isFirstQuestion ? "justify-end" : "justify-around"
          }`}
        >
