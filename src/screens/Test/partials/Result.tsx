...
const Result: FC<Props> = () => {
  ...
  const results = useMemo(() => {
    ...
    if (state.answers) {
      result = state.answers.reduce(
        (accumulator: any, item: UserAnswer) => {
  ...
  return (
    <TestBodyLayout
      title={translate("result_title").replace("__NAME__", state.name)}
    >
      <div className="w-full flex flex-row justify-center items-center mt-20">
        <p className="text-2xl italic text-white mr-10">
          {translate("introvert_label")}{" "}
          <span className="text-blue-50">{`${results?.inrtovert}%`}</span>
        </p>
        <p className="text-2xl italic text-white">
          {translate("extrovert_label")}{" "}
          <span className="text-blue-50">{`${results?.extrovert}%`}</span>
        </p>
      </div>
